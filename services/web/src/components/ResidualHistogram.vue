...
const option = computed(() => {
  ...
  return {
    grid: { left: 50, right: 16, top: 16, bottom: 40, containLabel: false },
    tooltip: {
    ...
    series: [
      {
        ...
        markLine: {
          ...
          data: [
            ...
            { xAxis: props.chemicalAccuracyEv, lineStyle: { color: '#f59e0b', type: 'dotted' as const, width: 1 } },
            { xAxis: -props.chemicalAccuracyEv, lineStyle: { color: '#f59e0b', type: 'dotted' as const, width: 1 } },
          ],
        },
      },
...
<template>
  <div class="rounded-xl border border-surface-200 bg-surface-0 p-4 shadow-sm dark:border-surface-800 dark:bg-surface-950">
    <header class="mb-3 flex items-baseline justify-between">
      <h3 class="text-sm font-semibold">{{ title ?? 'Distribuição de resíduos' }}</h3>
      <span class="text-[11px] text-surface-500">linhas: 0 (zero erro) e ±43 meV (chem. accuracy)</span>
    </header>
