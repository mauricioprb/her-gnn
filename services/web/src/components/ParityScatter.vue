...
const option = computed(() => {
  ...
  return {
    grid: { left: 60, right: 24, top: 20, bottom: 50, containLabel: false },
    tooltip: {
...
<template>
  <div class="rounded-xl border border-surface-200 bg-surface-0 p-4 shadow-sm dark:border-surface-800 dark:bg-surface-950">
    <header class="mb-3 flex items-baseline justify-between">
      <div>
        <h3 class="text-sm font-semibold">{{ title ?? 'Parity (pred vs DFT)' }}</h3>
        <p v-if="stats" class="mt-0.5 text-xs text-surface-500">
          n={{ stats.n }} · R²={{ stats.r2.toFixed(3) }} · MAE={{ stats.mae.toFixed(3) }} eV · RMSE={{ stats.rmse.toFixed(3) }} eV
        </p>
      </div>
    </header>
